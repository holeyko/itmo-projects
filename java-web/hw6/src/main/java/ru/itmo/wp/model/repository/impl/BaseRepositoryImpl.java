...
public abstract class BaseRepositoryImpl<T> implements BaseRepository<T> {
    ...
    @Override
    public List<T> findBy(SqlQueryImpl queryBuilder) {
    ...
    @Override
    public int findCount(SqlQueryImpl queryBuilder) {
        int result = 0;
        try (Pair<PreparedStatement, ResultSet> queryResponse = DatabaseUtils.executeSqlReadQuery(queryBuilder)) {
            ResultSet resultSet = queryResponse.getSecond();
            if (resultSet.next()) {
    ...
    @Override
    public long save(SqlQueryImpl queryBuilder) {
        try (Pair<PreparedStatement, Integer> queryResponse = DatabaseUtils.executeSqlModifyQuery(queryBuilder)){
            PreparedStatement statement = queryResponse.getFirst();
