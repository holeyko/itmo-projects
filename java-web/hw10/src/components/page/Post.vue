<template>
    <article>
        <div class="title"> <a href="#" @click.prevent="onViewPost(post.id)">{{ post.title }}</a></div>
...
export default {
    name: "Post",
    props: ["post", "users", "commentsCount"],
    methods: {
        onViewPost: function (id) {
            this.$root.$emit("onViewPost", id);
        }
    }
}
</script>
