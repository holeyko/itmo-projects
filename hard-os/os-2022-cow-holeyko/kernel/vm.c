  ...
  while (len > 0) {
    ...
    if (*pte & PTE_WW) {
      if (cowcopy(pagetable, va0) < 0) {
        panic("copyout: cow copy was failed");
      } else {
        pa0 = walkaddr(pagetable, va0);
      }
    }
...
void vmprint_ptes(pagetable_t pagetable, int level) {
  // there are 2^9 = 512 PTEs in a page table.
  for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    uint64 child = PTE2PA(pte);
    ...
    if ((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0) {
      vmprint_ptes((pagetable_t)child, level + 1);
    }
  }
}

void vmprint(pagetable_t pagetable) {
  printf("page table %p\n", pagetable);  // print page table virtual address
  vmprint_ptes(pagetable, 1);
}

void lockflags(uint *flags) {
  if (*flags & PTE_W) {
    *flags |= PTE_WW;
    *flags &= ~PTE_W;
  }
...
int cowcopy(pagetable_t pagetable, uint64 va) {
  ...
  flags = PTE_FLAGS(*pte);
  if ((flags & PTE_V) == 0 || (flags & PTE_WW) == 0) {
    return -1;
  }
