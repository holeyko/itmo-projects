...
// Invert a bit at position index in the array
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    int prev_alloc = bit_isset(bd_sizes[k].alloc, bi / 2);
    bit_invert(bd_sizes[k].alloc, bi / 2);  // free p at size k
    if (prev_alloc == 0) {                  // is buddy allocated?
...
int bd_initfree_pair(int k, int bi, int side) {
  int left, right;
  if (bi % 2 == 0) {
    left = bi;
    right = bi + 1;
  ...
  if (bit_isset(bd_sizes[k].alloc, bi / 2)) {
    ...
    free = BLK_SIZE(k);
    if (side == 0) {
      lst_push(&bd_sizes[k].free, addr(k, right));  // put right on free list
    } else {
      lst_push(&bd_sizes[k].free, addr(k, left));  // put left on free list
    }
  }
  return free;
}
